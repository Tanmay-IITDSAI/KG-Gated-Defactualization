...
Required environment variables (in .env):
    GROQ_API_KEY=gsk_...
    LOCAL_MODEL_NAME=meta-llama/Llama-3.1-8B-Instruct   # optional — falls back to Groq
    STEER_LAYER=20
    STEER_ALPHA=13.0
    HF_TOKEN=hf_...   # required — Llama-3.1-8B-Instruct is a gated model
"""
...
STEER_LAYER      = int(os.getenv("STEER_LAYER", "20"))    # 8B has 32 layers; layer 20 ≈ 63% depth
STEER_ALPHA      = float(os.getenv("STEER_ALPHA", "13.0")) # hidden=4096; slightly conservative vs Llama-2-7b's 15.0
...
def _run_kg_activation_steering(defactualized_prompt: str, style: str, kg: SupportKG) -> str:
    ...
    if steer_vec is None:
        raise RuntimeError(
            ...
            "Run build_vectors_llama31.ipynb and copy .style_cache/ to this folder."
        )
    ...
    # Llama-3.1-Instruct uses the header/eot prompt format — NOT [INST]<<SYS>>.
    ...
    # Llama-3.1 has two EOS tokens: <|eot_id|> (128009) and <|end_of_text|> (128001).
    # Passing both prevents runaway generation.
