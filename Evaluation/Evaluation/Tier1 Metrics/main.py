...
Supports THREE model backends (select via LOCAL_MODEL_NAME):
  • meta-llama/Llama-3.2-3B-Instruct  (default; ~6 GB VRAM in bfloat16)
  • meta-llama/Llama-3.2-1B-Instruct  (~2 GB VRAM in bfloat16)
  • meta-llama/Prompt-Guard-86M        (prompt-injection guard; no steering)
...
Required environment variables (in .env):
    GROQ_API_KEY=gsk_...
    LOCAL_MODEL_NAME=meta-llama/Llama-3.2-3B-Instruct   # or 1B / Prompt-Guard-86M
    STEER_LAYER=16   # 3B: 28 layers → layer 16; 1B: 16 layers → layer 10
    STEER_ALPHA=12.0
    HF_TOKEN=hf_...   # required — Llama-3.2 models are gated

Model settings reference:
  Model                              STEER_LAYER  STEER_ALPHA  Hidden  Layers  VRAM
  Llama-3.2-3B-Instruct             16           12.0         3072    28      ~6 GB
  Llama-3.2-1B-Instruct             10           10.0         2048    16      ~2 GB
  Prompt-Guard-86M                  N/A          N/A          768     12      <1 GB
"""
...
# Layer / alpha defaults auto-adapt to the chosen model at runtime (see _resolve_steer_config).
# You can always override via env vars:
#   Llama-3.2-3B-Instruct  → STEER_LAYER=16  STEER_ALPHA=12.0  (28 layers, hidden=3072)
#   Llama-3.2-1B-Instruct  → STEER_LAYER=10  STEER_ALPHA=10.0  (16 layers, hidden=2048)
#   Prompt-Guard-86M       → steering not applicable (classification model)
STEER_LAYER      = int(os.getenv("STEER_LAYER", "0"))     # 0 = auto-resolve below
STEER_ALPHA      = float(os.getenv("STEER_ALPHA", "0.0")) # 0.0 = auto-resolve below

# ── Per-model defaults ────────────────────────────────────────────────────────
_MODEL_DEFAULTS: dict[str, dict] = {
    "meta-llama/Llama-3.2-3B-Instruct": {"layer": 16, "alpha": 12.0, "hidden": 3072, "layers": 28},
    "meta-llama/Llama-3.2-1B-Instruct": {"layer": 10, "alpha": 10.0, "hidden": 2048, "layers": 16},
    "meta-llama/Prompt-Guard-86M":       {"layer":  0, "alpha":  0.0, "hidden":  768, "layers": 12},
}

def _resolve_steer_config() -> tuple[int, float]:
    """Return (steer_layer, steer_alpha), honouring env-var overrides."""
    defaults = _MODEL_DEFAULTS.get(LOCAL_MODEL_NAME, {"layer": 16, "alpha": 12.0})
    layer = STEER_LAYER if STEER_LAYER != 0    else defaults["layer"]
    alpha = STEER_ALPHA if STEER_ALPHA != 0.0  else defaults["alpha"]
    return layer, alpha
...
def _get_layer(model):
    ...
    steer_layer, _ = _resolve_steer_config()
    if hasattr(model, "model") and hasattr(model.model, "layers"):
        n = len(model.model.layers)
        assert steer_layer < n, f"STEER_LAYER={steer_layer} >= model layers={n}"
        return model.model.layers[steer_layer]
    if hasattr(model, "transformer") and hasattr(model.transformer, "h"):
        n = len(model.transformer.h)
        assert steer_layer < n, f"STEER_LAYER={steer_layer} >= model layers={n}"
        return model.transformer.h[steer_layer]
    # BERT-style (Prompt-Guard-86M uses DeBERTa/BERT encoder)
    if hasattr(model, "encoder") and hasattr(model.encoder, "layer"):
        n = len(model.encoder.layer)
        assert steer_layer < n, f"STEER_LAYER={steer_layer} >= encoder layers={n}"
        return model.encoder.layer[steer_layer]
...
def _get_activation(model, tokenizer, text: str):
    ...
    try:
        ...
        # Llama-3.2 uses the same <|begin_of_text|> format as 3.1 —
        # special tokens are already embedded so add_special_tokens=False.
        is_prompt_guard = "Prompt-Guard" in LOCAL_MODEL_NAME
        add_special = True if is_prompt_guard else False
        inputs = tokenizer(
...
def _run_kg_activation_steering(defactualized_prompt: str, style: str, kg: SupportKG) -> str:
    ...
    # Prompt-Guard-86M is a classification model — it cannot generate text.
    if "Prompt-Guard" in LOCAL_MODEL_NAME:
        raise RuntimeError(
            "Prompt-Guard-86M is a classification model and cannot generate text. "
            "Use it via run_prompt_guard_check() instead. "
            "Switch LOCAL_MODEL_NAME to Llama-3.2-3B-Instruct for generation."
        )
    ...
    if steer_vec is None:
        raise RuntimeError(
            ...
            "Run build_vectors_llama32.ipynb and copy .style_cache/ to this folder."
        )

    _, steer_alpha = _resolve_steer_config()
    ...
    # Llama-3.2-Instruct uses the same <|begin_of_text|> / header / eot_id format as 3.1.
    ...
    # add_special_tokens=False — special tokens already embedded in the prompt string
    inputs = tokenizer(full_prompt, return_tensors="pt", padding=False,
                       return_attention_mask=True, add_special_tokens=False).to(device)
    input_ids = inputs.input_ids
    target_layer = _get_layer(model)

    # Llama-3.2 has the same two EOS tokens as 3.1:
    #   <|end_of_text|> (128001) and <|eot_id|> (128009).
    ...
    try:
        with torch.no_grad():
            output_ids = model.generate(
                ...
                eos_token_id=_eos_ids,   # both <|end_of_text|> and <|eot_id|>
            )
    finally:
        handle.remove()
...
# ── Prompt-Guard-86M: jailbreak / injection classification ───────────────────

def run_prompt_guard_check(text: str) -> dict:
    """
    Classify ``text`` using Prompt-Guard-86M.

    Returns a dict with keys:
        label  : "JAILBREAK" | "INJECTION" | "BENIGN"
        score  : float (0–1 confidence of predicted label)
        all_scores : dict mapping every label to its probability
    """
    import torch
    from transformers import AutoTokenizer, AutoModelForSequenceClassification

    pg_name = "meta-llama/Prompt-Guard-86M"
    hf_tok  = os.getenv("HF_TOKEN", "")
    pg_tok  = AutoTokenizer.from_pretrained(pg_name, token=hf_tok)
    pg_mod  = AutoModelForSequenceClassification.from_pretrained(
        pg_name, token=hf_tok, torch_dtype=torch.bfloat16
    ).eval()

    inputs = pg_tok(text, return_tensors="pt", truncation=True, max_length=512)
    with torch.no_grad():
        logits = pg_mod(**inputs).logits
    probs  = torch.softmax(logits, dim=-1)[0]
    labels = pg_mod.config.id2label
...
def run_support_agent(complaint_data: dict, target_style: str, llm) -> dict:
    ...
    logger.info(f"[Support] style='{target_style}' scenario='{scenario}'")
    ...
    if not raw_response:
        logger.info(f"[Support] KG prompt steering fallback → style='{target_style}'")
        ...
        steering_mode = "kg_prompt_steered"
        logger.info("[Support] KG prompt steering SUCCESS")
    ...
    return {
        ...
        "defactualized_prompt": defactualized_message,
...
def make_comparison_table(emp_response: dict, form_response: dict) -> Table:
    ...
    table.add_row("KG nodes",
                  str(len(emp_response.get("kg_summary",{}).get("nodes",[]))),
                  str(len(form_response.get("kg_summary",{}).get("nodes",[]))))
    table.add_row("KG edges",
                  str(len(emp_response.get("kg_summary",{}).get("edges",[]))),
                  str(len(form_response.get("kg_summary",{}).get("edges",[]))))
...
async def run_pipeline(scenario: str, llm, llm_customer) -> dict:
    ...
    Returns a dict with all inputs, KG, and both pipeline outputs.
    """
    # Step 1: Generate customer complaint
    complaint_data = run_customer_agent(scenario, llm_customer)

    # Step 2: Run both support pipelines concurrently
    loop = asyncio.get_event_loop()
    emp_future  = loop.run_in_executor(None, run_support_agent, complaint_data, "empathetic", llm)
    form_future = loop.run_in_executor(None, run_support_agent, complaint_data, "formal",     llm)
    emp_response, form_response = await asyncio.gather(emp_future, form_future)

    # Display progress to console
    console.print(make_customer_panel(complaint_data))
    console.print(Columns([
        make_response_panel(emp_response,  "Empathetic — Support Response", "green"),
        make_response_panel(form_response, "Formal — Support Response",     "blue"),
    ]))
    ...
    record = {
        ...
        # ── Knowledge Graph ────────────────────────────────────────────────────
        "knowledge_graph": emp_response.get("kg_summary", {}),
        # ── Empathetic pipeline output ─────────────────────────────────────────
        "empathetic_output": {
            "support_response":     emp_response.get("support_response", ""),
            "defactualized_prompt": emp_response.get("defactualized_prompt", ""),
            "raw_steered_output":   emp_response.get("raw_steered_output", ""),
            "target_style":         emp_response.get("target_style", ""),
            "steering_mode":        emp_response.get("steering_mode", ""),
        },
        # ── Formal pipeline output ─────────────────────────────────────────────
        "formal_output": {
            "support_response":     form_response.get("support_response", ""),
            "defactualized_prompt": form_response.get("defactualized_prompt", ""),
            "raw_steered_output":   form_response.get("raw_steered_output", ""),
            "target_style":         form_response.get("target_style", ""),
            "steering_mode":        form_response.get("steering_mode", ""),
        },
    }
    return record
...
async def run_batch():
    ...
    console.print()
    console.print(Rule("[bold]A2A Customer Support — 100-Case Batch Run[/bold]"))
    console.print(
